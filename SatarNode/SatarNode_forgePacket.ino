...
void forgePacket(unsigned long timeStampEvent, unsigned int typeEvent, unsigned int ID) {    
  ...
  payloadString+="TSN="; // append the TSN variable to the empty string
  ...
  if (DEBUG) {
    Serial.println("debug information: timer, time_event, timeEvent, timeString");
  ...
  if (DEBUG) {
    ...
    Serial.println("debug information: packet forging and transaction time in microseconds");
